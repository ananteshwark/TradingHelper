...
Order of operations (each step is reported, nothing is silently skipped):
  ...
  4. normalise within peers and compute the composite;
  5. evaluate red flags (hard filters) and assign tiers;
  6. attach top-5 contributions with raw value, peer percentile and source
     filings, and the plain-language explanation.
"""
...
from igs.score import explain
from igs.score.normalize import composite, factor_long, normalise
from igs.score.red_flags import evaluate
...
def assign_tiers(results: pl.DataFrame, flags: pl.DataFrame, sc: ScoringConfig) -> pl.DataFrame:
    tripped = (flags.filter(pl.col("status") == "tripped").group_by("company_id")
                    .agg(pl.col("flag").str.replace_all("_", " ").str.join(", ")
                         .alias("tripped_flags")))
    unavailable = (flags.filter(pl.col("status") == "data_unavailable").group_by("company_id")
                        .agg(pl.len().alias("n_unavailable")))
    r = (results.join(tripped, on="company_id", how="left")
                .join(unavailable, on="company_id", how="left")
                .with_columns(pl.col("n_unavailable").fill_null(0)))
    eligible = pl.col("tripped_flags").is_null() & pl.col("composite").is_not_null()
    ...
    t = sc.tiers
    r = r.with_columns(
        pl.when(pl.col("tripped_flags").is_not_null()).then(pl.lit(REJECTED))
          .when(pl.col("composite").is_null()).then(pl.lit(REJECTED))
          .when((pl.col("rank_pct") <= t.high_conviction_top_pct / 100)
                & (pl.col("coverage") >= t.high_conviction_min_coverage)
                & (pl.col("n_unavailable") == 0)).then(pl.lit(HIGH))
          .when(pl.col("rank_pct") <= t.watchlist_top_pct / 100).then(pl.lit(WATCH))
          .otherwise(pl.lit(NOT_SHORTLISTED)).alias("tier"),
        pl.when(pl.col("tripped_flags").is_not_null())
          .then(pl.format("red flag: {}", pl.col("tripped_flags")))
          ...
          .then(pl.format("insufficient data (coverage {})",
                          ...
                          .cast(pl.Utf8) + "%"))
          .when((pl.col("rank_pct") <= t.high_conviction_top_pct / 100)
                & (pl.col("n_unavailable") > 0))
          .then(pl.lit("top-ranked, but some red flags could not be checked"))
          .when((pl.col("rank_pct") <= t.high_conviction_top_pct / 100)
                & (pl.col("coverage") < t.high_conviction_min_coverage))
          .then(pl.lit("top-ranked, but factor coverage is below the threshold"))
...
def score(dataset: PitDataset, as_of: dt.datetime, sc: ScoringConfig, uc: UniverseConfig,
    ...
    companies = inc["company_id"].to_list()
    flags = evaluate(view, companies, rf)
    results = (inc.join(res.composite, on="company_id", how="left"))
    results = assign_tiers(results, flags, sc)
    view.audit.assert_clean()
    ...
    factors = res.factors.with_columns(
    ...
    return ScoreRun(as_of=as_of, universe=universe, results=results, pillars=res.pillars,
                    factors=factors, flags=flags, dropped_factors=sorted(dropped),
                    ic_status_generated_at=ic_generated, gate_fingerprint=gate, dq=dq)
